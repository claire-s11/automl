...
def batch_image_preprocess(raw_images,
  ...
  if not batch_size:
    ...
    images, scales = tf.map_fn(
        map_fn, raw_images, dtype=(tf.float64, tf.float64), back_prop=False)
...
def build_model(model_name: Text, inputs: tf.Tensor, **kwargs):
  ...
  if kwargs.get('precision', None):
    # Post-processing has multiple places with hard-coded float64.
    # TODO(tanmingxing): Remove them once post-process can adpat to dtypes.
    cls_outputs = {k: tf.cast(v, tf.float64) for k, v in cls_outputs.items()}
    box_outputs = {k: tf.cast(v, tf.float64) for k, v in box_outputs.items()}
...
def det_post_process_combined(params, cls_outputs, box_outputs, scales,
  ...
  classes = tf.cast(nmsed_classes + 1, tf.float64)
